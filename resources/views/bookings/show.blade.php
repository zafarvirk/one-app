            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-0">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="bs-stepper list-group-item">
                            <div class="bs-stepper-header" role="tablist">
                                @foreach($transactionStatuses as $transactionStatus)
                                    <div class="step">
                                        <span role="tab">
                                            <span class="bs-stepper-circle @if($transactionStatus->id == $booking->booking_status_id) bg-{{setting('theme_color')}} @endif">{{$transactionStatus->order}}</span>
                                            <span class="bs-stepper-label">{{$transactionStatus->status}}</span> </span>
                                    </div>
                                    @if (!$loop->last)
                                        <div class="line"></div>
                                    @endif
                        ...
                        @foreach($booking->article as $article)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! getMediaColumn($article,'image','rounded shadow-sm border') !!}
                                <div class="d-flex flex-column mx-3">
                                    <b>{{$article->name}}</b> <small>{{__('lang.by')}} {{$booking->business->name}}</small>
                                </div>
                                <div class="text-bold ml-xl-auto my-1 my-xl-0">
                                    @if($article->hasDiscount())
                                        <del class="text-gray">{!! getPrice($article->price) !!}</del>
                                    @endif
                                    <span class="h5 text-bold">{!! getPrice($article->getPrice()) !!}</span>
                                </div>
                            </li>
                        @endforeach
                        <li class="list-group-item bg-light">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body px-0 py-1">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <b>{{__('lang.duration')}}</b><small>{{$booking->getDurationInHours()}} {{__('lang.booking_hours')}}</small>
                        </li>
                    </ul>
                </div>
            </div>
