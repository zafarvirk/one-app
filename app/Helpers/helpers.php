 ...
 * @return null|string|string[]
 */
function getDaysColumn($modelObject)
{
    $html = "";
    foreach ($modelObject->days as $day) {
        $html .= '<span data-toggle="tooltip" data-placement="left" title=".$day.">'.$day.'</span><br>';
    }
...
function notify($data, $user , $title){
    $fields = array(
        'filters' => array(
            array(
            "field" => "tag",
    ...
    return OneSignal::sendPush($fields , $title);
}
