...
class UpdateBookingEarningTable
{
    ...
    public function handle($event)
    {
        try {
            ...
            $this->bookingRepository->pushCriteria(new BookingsOfSalonCriteria($business->id));
            ...
            $this->earningRepository->updateOrCreate(['business_id' => $business->id], [
                    'total_bookings' => $bookingsCount,
                    'total_earning' => $total - $tax,
                    'taxes' => $tax,
                    'admin_earning' => ($total - $tax) * (100 - $business->business_category->commission) / 100,
                    'business_earning' => ($total - $tax) * $business->business_category->commission / 100,
                ]
            );
