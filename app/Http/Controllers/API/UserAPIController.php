...
class UserAPIController extends Controller
{
    ...
    function login(Request $request)
    {
        try {
            ...
            $login_type = strtolower($request->input('login_type'));
            ...
            if($login_type != 'email' && $login_type != 'phone') {
                ...
                if(count($user) > 0){
                    ...
                    $user->password = Hash::make(112211);
                    $user->api_token = Str::random(60);
                    $user->access_token = $request->input('access_token');
                ...
                if (auth()->attempt(['phone_number' => $request->input('phone_number'), 'password' => $password])) {
    ...
    function isUserExist(Request $request): JsonResponse
    {
        $this->validate($request, [
            'data' => 'required',
        ]);
        if($request->data == '' || $request->data == null){
            return $this->sendError('Invalid data or empty', 200);
        }
        try {
            if ( $this->checkEmail($request->data) ) {
                $user = $this->userRepository->findByField('email', $request->input('data'))->first();
                ...
                $user = $this->userRepository->findByField('phone_number', $request->input('data'))->first();
