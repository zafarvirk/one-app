...
class CartAPIController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $exist = $this->cartRepository->where(['user_id'=> $input['user_id'] , 'article_id' => $input['article_id']])->first();
            if($exist){
                $input['quantity'] += $exist->quantity;
                $input['price'] = (($exist->quantity + $request->quantity) * $request->price);
                $cart = $this->cartRepository->update($input, $exist->id);
                return $this->sendResponse($cart->toArray(), 'Item updated successfully');
            }
            else{
                $input['price'] = ($request->price * $request->quantity);
