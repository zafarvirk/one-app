...
class GalleryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('status', function ($gallery) {
                return getBooleanColumn($gallery, 'status');
            })
            ->editColumn('business.name', function ($gallery) {
                return getLinksColumnByRouteName([$gallery->business], 'salons.edit', 'id', 'name');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            [
                'data' => 'status',
                'title' => trans('lang.gallery_status'),

            ],
            [
                'data' => 'business.name',
                'name' => 'business.name',
                'title' => trans('lang.gallery_business_id'),
            ],
            [
                'data' => 'updated_at',
    ...
    public function query(Gallery $model)
    {
        if (auth()->user()->hasRole('salon owner')) {
            return $model->newQuery()->with("business")
                ->join('business_users', 'business_users.business_id', '=', 'galleries.business_id')
            ...
            return $model->newQuery()->with("business")->select("$model->table.*");
        }
    }
