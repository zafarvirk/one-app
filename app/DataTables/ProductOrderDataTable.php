...
class ProductOrderDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ->editColumn('article.name', function ($productOrder) {
                return $productOrder->article['name'];
            })
     ...
     */
    public function query(ArticleOrder $model)
    {
        return $model->newQuery()->with("article")
            ->where('article_orders.order_id', $this->id)
            ->select('article_orders.*')->orderBy('article_orders.id', 'desc');
