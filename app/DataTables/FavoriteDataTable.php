...
class FavoriteDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('article.name', function ($favorite) {
                return getLinksColumnByRouteName([$favorite->article], 'article.edit', 'id', 'name');
            })
    ...
    public function query(Favorite $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("article")->with("user")->select("$model->table.*");
        } else {
            return $model->newQuery()->with("article")->with("user")
                ->join("article", "article.id", "=", "favorites.article_id")
