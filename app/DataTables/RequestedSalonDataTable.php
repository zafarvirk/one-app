...
class RequestedSalonDataTable extends DataTable
{
    ...
    public function query(Business $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("business_category")->with("address")->where('businesses.accepted', '0')->select("$model->table.*");
        } else {
            return $model->newQuery()
                ->with("business_category")
                ->join("business_users", "business_id", "=", "businesses.id")
                ->where('business_users.user_id', auth()->id())
                ->where('businesses.accepted', '0')
